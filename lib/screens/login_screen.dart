...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void login() {
    ...
    if (phoneController.text.isEmpty) {
      ToastService().showToast(
        message: 'دخل رقم تليفونك يا نجم',
        type: ToastType.error,
      );
      return;
    }
    ...
    if (!RegExp(r'^\d{11,}$').hasMatch(phoneController.text)) {
      ToastService().showToast(
        message: 'اكتب رقم تليفون صح كده يا نجم',
        type: ToastType.error,
      );
      return;
    }
    ...
    if (passwordController.text.length < 8) {
      ToastService().showToast(
        message: 'الباسورد لازم تكون ٨ حروف على الأقل يا نجم',
        type: ToastType.error,
      );
      return;
    }
      ...
      body: BlocListener<AuthenticationCubit, AuthenticationState>(
        listener: (context, state) {
          if (state is AuthenticationSignInSuccess) {
            ...
            ToastService().showToast(
              message: state.msg.endsWith("يا نجم") ? state.msg : "${state.msg} يا نجم",
              type: ToastType.error,
            );
          }
        },
